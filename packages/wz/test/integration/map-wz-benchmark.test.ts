...
describe.skipIf(!present)(`Map.wz walk benchmark vs @tybys/wz`, () => {
  ...
  it(
    'walks Map/Map0 image bucket and reports wall-clock vs the oracle',
    async () => {
      ...
      const sample = mapBucket.children
        .filter((c): c is { kind: 'image'; name: string; offset: number; fileSize: number; checksum: number } => c.kind === 'image')
        .slice(0, 50);
